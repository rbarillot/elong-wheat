...
"""
# TODO: @Christope_Fabrice : la liste des inputs/outputs attendus par le modele. Cartains sont communs entre version et d'autres propres à la version hydraulique. Comment gérer ca sachant que ces variables sont appelées à de nombreux endroits?
...
HIDDENZONE_INPUTS_hydraulics = ['leaf_is_growing', 'internode_is_growing', 'leaf_pseudo_age', 'internode_pseudo_age', 'leaf_pseudostem_length', 'internode_distance_to_emerge', 'leaf_L', 'internode_L',
                     'lamina_Lmax', 'width', 'thickness', 'sheath_Lmax', 'temperature', 'turgor_water_potential',
                     'hiddenzone_age', 'SSLW', 'LSSW', 'leaf_is_emerged', 'internode_Lmax', 'internode_Lmax_lig', 'LSIW',
                     'internode_is_visible', 'sucrose', 'amino_acids', 'fructan', 'proteins', 'leaf_enclosed_mstruct', 'leaf_enclosed_Nstruct', 'internode_enclosed_mstruct',
                     'internode_enclosed_Nstruct', 'mstruct', 'is_over', 'mean_conc_sucrose', 'sheath_is_emerged']
ELEMENT_INPUTS = ['length', 'Wmax', 'is_growing', 'age', 'age_teq']
ELEMENT_INPUTS_hydraulics = ['length', 'is_growing', 'age']
AXIS_INPUTS = ['SAM_temperature', 'delta_teq', 'teq_since_primordium', 'status', 'nb_leaves', 'GA', 'SAM_height', 'cohort', 'sum_TT']
AXIS_INPUTS_hydraulics = ['xylem_water_potential', 'SAM_temperature', 'delta_teq', 'teq_since_primordium', 'status', 'nb_leaves', 'GA', 'SAM_height', 'cohort', 'sum_TT']
...
HIDDENZONE_OUTPUTS_hydraulics = ['sheath_is_emerged', 'res', 'delta_teq', 'leaf_is_growing', 'internode_is_growing', 'leaf_pseudo_age', 'delta_leaf_pseudo_age', 'internode_pseudo_age', 'delta_internode_pseudo_age', 'leaf_pseudostem_length', 'lamina_Lmax',
                      'sheath_Lmax', 'temperature', 'SL_ratio', 'leaf_enclosed_mstruct', 'init_leaf_L', 'width', 'thickness',
                      'delta_internode_distance_to_emerge', 'leaf_L', 'delta_leaf_L', 'internode_L','delta_internode_L',
                      'SSLW', 'LSSW', 'leaf_is_emerged', 'internode_Lmax', 'internode_Lmax_lig', 'LSIW', 'internode_is_visible', 'sucrose', 'amino_acids', 'fructan', 'proteins',
...
ELEMENT_OUTPUTS_hydraulics = ['length', 'is_growing', 'sucrose', 'amino_acids', 'fructan', 'proteins', 'nitrates', 'starch', 'cytokinins', 'width', 'thickness',
...
class Simulation(object):
    ...
    def run(self, Tair, Tsoil, Zsowing=0.025, optimal_growth_option=False, hydraulics=False):
        """
        Run the simulation.
            ...
            #: Main Stem
                ...
                if curr_hiddenzone_outputs['leaf_is_growing']:
                    ...
                    if not prev_leaf_emerged and phytomer_id != 1:  #: Before the emergence of the previous leaf. Exponential-like elongation. Leaf1 starts directly in phase 2
                        # delta leaf length
                        delta_leaf_L = model.calculate_deltaL_preE(hiddenzone_inputs['sucrose'],
                                                                   hiddenzone_inputs['leaf_L'],
                                                                   hiddenzone_inputs['amino_acids'],
                                                                   hiddenzone_inputs['mstruct'],
                                                                   curr_axis_outputs['delta_teq'], phytomer_id,
                                                                   optimal_growth_option,
                                                                   xylem_water_potential=axis_inputs.get('xylem_water_potential')) # TODO: @Christope_Fabrice
                        ...
                        if not hydraulics:
                            ...
                            leaf_pseudo_age = model.calculate_leaf_pseudo_age(hiddenzone_inputs['leaf_pseudo_age'], curr_axis_outputs['delta_teq'], turgor_water_potential=hiddenzone_inputs['turgor_water_potential'])
                            delta_leaf_L = 0
                            leaf_L = hiddenzone_inputs['leaf_L']

                            # Ratio (mass) of Division Zone in the hiddenzone
                            curr_hiddenzone_outputs['ratio_DZ'] = model.calculate_ratio_DZ_postE_hydraulics(leaf_L, leaf_pseudo_age, leaf_pseudostem_length)
                        ...
                        if not curr_hiddenzone_outputs['leaf_is_emerged']:
                            ...
                            if curr_hiddenzone_outputs['leaf_is_emerged']:  # Initialise lamina outputs
                                ...
                                if next_hiddenzone_id in all_hiddenzone_inputs:
                                    ...
                                    if not hydraulics:
                                        ...
                                        next_hiddenzone_outputs['lamina_Lmax'] = model.calculate_lamina_Lmax_hydraulics(hiddenzone_id[2])  #: Maximal lamina length
                            ...
                            if leaf_L >= curr_hiddenzone_outputs['leaf_Lmax'] and not hydraulics:
                            ...
                            # Test end of elongation
                                if (lamina_L >= curr_hiddenzone_outputs['lamina_Lmax']) or (leaf_L >= curr_hiddenzone_outputs['leaf_Lmax']):
                                    ...
                                    if hidden_lamina_L > 0:
                                        ...
                                        curr_hiddenzone_outputs['leaf_is_growing'] = False
                                        curr_hiddenzone_outputs['leaf_is_remobilizing'] = True
                            # TODO: @Christope_Fabrice : il y a pas mal de choses répétées entre les 2 versions mais le 1er if est différent, dans 1 cas ca dépend d'une comparaison de longueur et dans l'autre d'age
                            else: # hydraulics
                                if curr_hiddenzone_outputs['leaf_pseudo_age'] >= parameters.te:
                                    ...
                                    self.outputs['elements'][hidden_sheath_id][
                                        'length'] = leaf_pseudostem_length  # Length of hidden sheath
                                    curr_hidden_sheath_outputs['width'] = all_hiddenzone_inputs[hiddenzone_id]['width']
                                    curr_hidden_sheath_outputs['thickness'] = all_hiddenzone_inputs[hiddenzone_id][
                                        'thickness']
                                    curr_hidden_sheath_outputs['is_growing'] = False
                                    self.outputs['elements'][
                                        hidden_sheath_id] = curr_hidden_sheath_outputs  # Update of sheath outputs
                                    ...
                                    curr_visible_sheath_outputs['length'] = max(1E-03,
                                                                                curr_hiddenzone_outputs['leaf_L'] -
                                                                                curr_lamina_outputs['length'] -
                                                                                self.outputs['elements'][
                                                                                    hidden_sheath_id]['length'])

                                    curr_visible_sheath_outputs['is_growing'] = False
                                    curr_visible_sheath_outputs['width'] = all_hiddenzone_inputs[hiddenzone_id]['width']
                                    curr_visible_sheath_outputs['thickness'] = all_hiddenzone_inputs[hiddenzone_id][
                                        'thickness']
                                    self.outputs['elements'][
                                        visible_sheath_id] = curr_visible_sheath_outputs  # Update of sheath outputs

                                    # Lamina and sheath Lmax
                                    curr_hiddenzone_outputs['lamina_Lmax'] = curr_lamina_outputs['length']
                                    curr_hiddenzone_outputs['sheath_Lmax'] = curr_visible_sheath_outputs['length'] + \
                                                                             curr_hidden_sheath_outputs['length']
                                    ...
                                    if next_hiddenzone_id in all_hiddenzone_inputs:
                                        next_hiddenzone_outputs = all_hiddenzone_outputs[next_hiddenzone_id]
                                        if curr_axis_outputs['GA']:
                                            next_hiddenzone_outputs['internode_Lmax'] = model.calculate_internode_Lmax(
                                                next_hiddenzone_outputs[
                                                    'internode_L'])  #: Estimate of final internode length
                                            next_hiddenzone_outputs['internode_Lmax_lig'] = next_hiddenzone_outputs[
                                                'internode_Lmax']  #: Estimate of final internode length at previous leaf ligulation
                                        next_hiddenzone_outputs['LSIW'] = model.calculate_LSIW(
                                            next_hiddenzone_outputs['LSSW'], next_hiddenzone_id[2],
                                            optimal_growth_option=True)  #: Lineic Structural Internode Weight
                                        next_hiddenzone_outputs[
                                            'internode_pseudo_age'] = 0  #: Pseudo age of the internode since beginning of automate growth (s)
                                        self.outputs['hiddenzone'][next_hiddenzone_id] = next_hiddenzone_outputs
                                    else:
                                        warnings.warn(
                                            'No next hidden zone found for hiddenzone {}.'.format(hiddenzone_id))
                        ...
                        # Mature lamina, growing sheath
                            ...
                            if not hydraulics:
                                ...
                                lamina_L = \
                                    self.outputs['elements'][hiddenzone_id + tuple(['blade', 'LeafElement1'])]['length']
                                emerged_sheath_L = model.calculate_emerged_sheath_L(leaf_L, leaf_pseudostem_length,
                                                                                    lamina_L,
